...
description: Detect user-visible parameter-surface differences between source LLM provider and Bedrock. Loaded by ai-code-analyzer (Track 2) when source_provider is openai or gemini and same_model_family is false; consumed by ai-code-rewriter to confirm each user-visible change with the user before modifying code.
---
...
- `source_provider ∈ {openai, gemini}` AND
...
| gemini | references/gemini-to-bedrock.md |
...
`resolution_chosen` is a typed enum: `"{option_set_id}_{1-indexed_option_number}"` for ux_choice and fallback paths, or `"impl_path_default"` for impl_path defaults. The schema in `src/tool/report-completion-schemas.ts` is the authoritative list.
...
If during code rewrite (Step 3 of `ai-code-rewriter.txt`) you are about to modify a parameter affecting **user-visible behavior** that was NOT in `behavior_deltas`, STOP and do not modify it. The rewriter is non-interactive and cannot ask the user, so return `blocked` and record the situation in `notes`, naming the **`fallback` option set** above so the orchestration skill can present those exact options to the user. Do not improvise options in this path — the whole point of the fallback rule is the analyzer missed something, so improvising compounds the risk.

Same applies if the analyzer emitted a delta with an unrecognized `delta_type` or `option_set_id` (version skew between analyzer and rewriter): trigger the fallback rule and add an `errors:` note in the returned notes.

This is a safety net for grep-recipe gaps in this skill. Better to over-ask than recreate the original silent-change bug through a code path the analyzer missed.
