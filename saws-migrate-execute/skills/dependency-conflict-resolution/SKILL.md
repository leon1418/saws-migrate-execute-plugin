...
description: Allowlist gate that blocks the rewriter from removing customer-pre-existing dependencies during dependency edits. Load before committing manifest changes. Complements Step 5c lockfile regeneration; does not replace it.
---

# Dependency Conflict Resolution — Allowlist Gate

A **mechanical safety gate**, not a recipe book. Solver-driven verification already lives in `ai-code-rewriter.txt` Step 5c (`poetry lock`, `uv lock`, `npm install --package-lock-only`, etc.) and stop-and-report-on-conflict already lives in Step 5d. This skill adds **one** thing on top: prevent the agent from "fixing" a resolver failure by silently deleting a dependency the customer's project relied on before this rewrite session.

All commands run on the host against the repository checkout. `<REPO>` is the repository path supplied in your context.

## When to load

Load **before Step 7a's `git commit`**, after Step 5d has run lockfile regeneration cleanly. The gate inspects the working-tree manifest diff for removals and decides whether each removal is permitted.
...
The rewriter's Step 1 persists the baseline commit SHA in `/tmp/dcr-baseline-sha`. The gate uses it as the comparison anchor (a branch name like `bedrock-migration` would be wrong here — the rewriter works on that branch, so `bedrock-migration..HEAD` is empty by definition).

### Step A: list removed package names

Diff the working tree against the baseline SHA (not `HEAD`) so the gate catches removals across any intermediate commits between Step 1 and Step 7a:
...
Use `git diff $BASELINE` (same anchor as Step A) — `git log -p $BASELINE..HEAD` would miss everything because the gate runs before Step 7a's commit, so the rewriter's edits are still uncommitted in the working tree.
...
- Picking the right solver (`poetry lock` vs `uv lock` vs `npm install` etc.) — Step 5c handles that.
- Diagnosing or auto-fixing resolver failures — Step 5d's policy is "stop and report; do not modify the manifest further to make resolution succeed." This skill aligns with that policy and only adds a safety check on the *kind* of edit the agent is allowed to commit.
