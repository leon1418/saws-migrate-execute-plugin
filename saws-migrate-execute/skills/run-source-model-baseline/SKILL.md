...
- `<REPO>/.saws-migrate/.source-provider-env` exists (created by
  ai-code-analyzer Step 8.6 when the user provided a source-provider API
  key). The file contains a single `KEY=VALUE` line, one of:
...
Absent → return immediately with `status: "skipped"`. Present → continue.
