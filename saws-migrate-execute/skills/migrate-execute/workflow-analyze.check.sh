#!/usr/bin/env bash
# workflow-analyze.check.sh — verify the workflow is RUNTIME-COMPATIBLE.
#
...
# are NOT (a real e2e run hit `SyntaxError: import call expects ... arguments`).
# We compile the body the same way the runtime does — WITHOUT stripping imports —
# so a static import breaks the check (previously the check stripped imports and
# masked this exact bug). We also assert no `import `/`readFileSync`/`node:fs`.
set -euo pipefail
cd "$(dirname "$0")"
F=workflow-analyze.js
