...
description: Validate a plan-supplied Bedrock model ID against live inference profiles in the target AWS account and correct it if stale. Load this when a plan's target_model_id needs to be used for the first time (analyzer Step 8.5), or when a Bedrock call returns "invalid model identifier".
---
