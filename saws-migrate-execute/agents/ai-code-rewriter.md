...
You run INSIDE an isolated git worktree of the user's repository — the workflow runtime created it for you (`isolation: 'worktree'`). Your current working directory IS that isolated checkout. Branch, commit, and edit operations happen here without dirtying the user's working tree. You do NOT create your own worktree or Docker container — you are already in one. Run all commands directly via the `Bash` tool against the current working directory.
...
This agent runs ONLY for **Track 2** (AI-only → Bedrock), as phase **T2-5** in the migrate-execute pipeline. Track 1 (infrastructure migration) does not call you.
...
- **`<SOURCE_PATH>`** — the source code path. This is the current working directory (an isolated git worktree of the repository); see the `Repository:` line in your context.
...
- **`Confirmed behavior-delta decisions`** — a JSON array in your context. Each entry carries a behavior delta and the user's chosen resolution/option. §9 applies these EXACTLY as decided.
...
# 7. Create branch in the current working directory

You are already inside an isolated git worktree of the repository (the workflow runtime created it). Your branch `bedrock-migration` is created here, on top of the user's repo, WITHOUT dirtying the user's working tree. Do NOT create another worktree or container.

Before the baseline commit, strip junk that the upstream repo may have tracked (e.g., `.DS_Store` from macOS contributors, stray `__pycache__/` from a forgotten run). Otherwise that junk lives forever in `bedrock-migration` history — §16's `.gitignore` only stops *new* additions; it can't retroactively untrack what's already in HEAD. Clean both the git index (`git rm --cached`) and the working tree (`find ... -delete`); skipping the working-tree pass would let the next `git add -A` re-add them.

```bash
# Strip pre-existing junk from index AND working tree (no-op if absent)
...
git add -A && git commit -m 'baseline: pre-migration snapshot' --allow-empty 2>/dev/null || true
git tag -f saws-migrate-baseline HEAD
git rev-parse HEAD > /tmp/dcr-baseline-sha

# Branch-collision check: if the upstream repo already has a `bedrock-migration` branch
# (returning customer / prior failed run / naming coincidence), `checkout -b` would fail.
# Detect first, then decide rather than silently overwriting.
git rev-parse --verify bedrock-migration 2>/dev/null && echo BRANCH_EXISTS || echo BRANCH_FREE
```

**On `BRANCH_FREE`** — proceed:

```bash
git checkout -b bedrock-migration
```

**On `BRANCH_EXISTS`** — the upstream repo already has a `bedrock-migration` branch. Do NOT silently delete and recreate it — the existing branch may be the customer's work in progress. Use a different, non-colliding name (e.g. `bedrock-migration-2`), proceed on that branch, and set `branch_name` in §27's payload accordingly. Record the collision and the name you used in `notes`. Reserve `blocked` only if you cannot make forward progress at all.
...
```bash
command -v pip-compile || pip install --quiet pip-tools
...
```bash
command -v poetry || pip install --quiet 'poetry>=1.7,<3'
...
```bash
command -v pdm || pip install --quiet pdm
...
**Step 1 — stage all changes** (so the skill in step 2 has a diff to read):
...
```bash
(git worktree remove --force /tmp/clean-checkout 2>/dev/null; git branch -D test-clean-checkout 2>/dev/null; true) && git worktree add -b test-clean-checkout /tmp/clean-checkout bedrock-migration
```
...
```bash
( cd /tmp/clean-checkout && grep -rPn "\"/(tmp|home|root|private/tmp)/" tests/ __tests__/ 2>/dev/null || echo "PORTABILITY GUARD PASSED: no machine-only filesystem paths found in tests" )
```
...
```bash
git checkout bedrock-migration && git merge test-clean-checkout --ff-only
```
...
Expected: at least two commits on `bedrock-migration` (code rewrite from §14, tests from §20). Working tree clean. Do NOT re-run `pytest`/`jest` here — tests already ran in §18 in the clean checkout, which is the only environment whose result counts.
...
Return your result as the structured object matching the rewrite schema (the workflow validates it automatically). If you hit a hard wall, return `{ blocked: { reason, detail } }`.

## What goes in the result

- **`data`** — the typed fields in the REWRITE schema: `branch_name`, `files_changed`, `dependencies_updated`, `notes`, `behavior_delta_decisions`. The runtime auto-attaches per-file `diffs` by reading git — you do NOT need to include a `diffs` field. The runtime requires a clean single-commit branch (§25 satisfies this).
...
Genuine hard stops (e.g. can't build, missing critical context) return `{ blocked: { reason, detail } }`. The rewrite schema's control state allows `reason` only from the shared enum (`model_access`, `source_key_auth`, `model_unresolvable`). If a blocker you hit doesn't fit one of those, prefer recording the problem in `notes` and continuing where safe — reserve `blocked` for true show-stoppers. (Use judgment; the workflow only checks `rewrite.blocked`.)
...
The runtime schema is `REWRITE`, defined inline in `skills/migrate-execute/workflow-rewrite.js` (the workflow validates your return against it). Extra keys are rejected.
